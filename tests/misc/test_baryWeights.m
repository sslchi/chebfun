...
% NOTE: We only test the consistency of barycentric weights equal 1 or -1.
...
[x, ~, v] = feval(xpts,5,a);  
vb = baryWeights(x); id = find(abs(vb) == 1);
pass(j+1) = norm( v(id)-vb(id),inf ) < eps; 

[x, ~, v] = feval(xpts,6,a);  
vb = baryWeights(x); id = find(abs(vb) == 1);
pass(j+2) = norm( v(id)-vb(id),inf ) < eps; 

[x, ~, v] = feval(xpts,2003,a);  
vb = baryWeights(x); id = find(abs(vb) == 1);
pass(j+3) = norm( v(id)-vb(id),inf ) < eps; 

[x, ~, v] = feval(xpts,2004,a);  
vb = baryWeights(x); id = find(abs(vb) == 1);
pass(j+4) = norm( v(id)-vb(id),inf ) < eps; 
...
[x, ~, v] = feval(xpts,2003,a,b);  
vb = baryWeights(x); id = find(abs(vb) == 1);
pass(j+3) = norm( v(id)-vb(id),inf ) < eps; 

[x, ~, v] = feval(xpts,2004,a,b);  
vb = baryWeights(x); id = find(abs(vb) == 1);
pass(j+4) = norm( v(id)-vb(id),inf ) < eps;
