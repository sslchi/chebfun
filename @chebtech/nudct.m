...
% The acos(x) function is numerically unstable to compute when x is close
% to +1 or -1. Therefore, for any evaluation points that fall in the region
% |x|>0.995, we do not use the fast transform as acos(x) cannot be computed
% accurately. 
magic_number = 0.995; 
idx = find(abs(x)>magic_number); 
...
if ( ~isempty(idx) )
    v(idx) = chebtech.clenshaw(x(idx),coeffs); 
end

% If the coefficients were real, then enforce it: 
if ( isreal(coeffs) )
    v = real(v);
