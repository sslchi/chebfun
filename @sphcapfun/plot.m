...
%             [XX,YY,ZZ] = sphere(101);
            [LL,TT] = meshgrid(linspace(dom(1), dom(2), 101), ...
            linspace(dom(3), dom(4), 101));
        ...
        if ( plotline )
            ...
            t1 = linspace(-pi, pi, num_t)';  % Parametric variable
            t2 = linspace(-dom(4), dom(4), num_t)';
            colCircs = [];
            rowCircs = [];
            baseRowCirc = [ cos(t1) sin(t1) ];
            for k=1:size(pivots, 1)
                ...
                if ( (abs(pivotsCart(k, 3) + 1) > 100*eps) && ...
                    ...
                    colCircs = [ colCircs; sin(t2)*r cos(t2) ];
                    ...
                    if ( ~isempty(colCircs) ) % No circle through poles yet
                        colCircs = [ colCircs; sin(t2)*[1 0 0] cos(t2) ];
                        colCircs = [ colCircs; nan nan nan ];
                    end
                end
            end
