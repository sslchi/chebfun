classdef sphcapfun < separableApprox
%SPHCAPFUN   SPHCAPFUN class for representing functions on the unit disk.
% 
%   Class for approximating smooth functions defined on the unit disk. The 
%   functions should be smooth. Sphcapfun objects can be constructed in a 
%   variety of ways: 
%
%    1. Y = sphcapfun(F), where F is a function handle in Cartesian 
%       coordinates (x,y), e.g., @(x,y) x.*y + cos(x).
%    2. Y = sphcapfun(F, 'polar'), where F is a function handle in polar 
...
%    3. Y = sphcapfun(F), where F is a matrix of numbers.
%
...
%
% SPHCAPFUN(F, k) returns a rank k approximation to F.
%
% SPHCAPFUN(F, [m n]) returns a representation of F using a degree m
...
% TODO: Include documentation of fixed eps construction
    ...
    methods ( Access = public, Static = false )
        ...
        function f = sphcapfun(varargin)
            % The main sphcapfun constructor!
            
            % Return an empty SPHCAPFUN:
            if ( (nargin == 0) || isempty(varargin{1}) )
                f.domain = [-pi pi 0 2*pi/3];
                return
            end
    ...
    methods ( Access = public, Static = false )
        ...
        % The main bulk of the SPHCAPFUN constructor:
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %% Private Static methods implemented by SPHCAPFUN class.
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    properties (Access = public)
        % DOMAIN: default is [-pi,pi] x [0,2*pi/3] which corresponds to using 
        % polar coordinates. 
