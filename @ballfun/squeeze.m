...
%SQUEEZE   Squeeze a ballfun to a CHEBFUN2 or a CHEBFUN if possible.
%   G = squeeze(F) returns a ballfun if F depends on all of the three 
...
if ( isempty(f) )     % Check for an empty ballfun.
...
% If f is of rank 1, then it MAY be a function of just ONE variable:
    ...
    % If cols and rows are constant then we have a function of z only.
        ...
        newCore = squeeze(ballfun.txm(ballfun.txm(core, cols.coeffs, 1), ...
             rows.coeffs, 1));
    ...
    elseif ( length(rows) == 1 &&  length(tubes) == 1 )
        ...
        newCore = squeeze(ballfun.txm(ballfun.txm(core, rows.coeffs, 2), ...
             tubes.coeffs, 3));
        newdomain = dom(1:2);
...
% and z.
    newCore = squeeze(ballfun.txm(core, cols.coeffs, 1));
    newdomain = dom(3:6);
...
% and z.
    newCore = squeeze(ballfun.txm(core, rows.coeffs, 2));
...
% y.
    newCore = squeeze(ballfun.txm(core, tubes.coeffs, 3));
    newdomain = dom(1:4);
