...
%   FEVAL(F,X,Y,Z) evaluates the ballfun F at the point(s) in (X,Y,Z), 
...
if ( isa(x, 'chebfun') && isa(y, 'chebfun') && isa(z, 'chebfun') )
    % Extract ballfun along the path x = x(t), y = y(t), z = z(t), i.e.,
...
elseif ( strcmpi(x, ':') && strcmpi(y, ':') && strcmpi(z, ':') )   % f(:, :, :)
    % Simply return the ballfun object itself.
    out = f;
...
elseif ( isnumeric(x) && strcmpi(y, ':') &&  strcmpi(z, ':') ) % f(x, :, :)
    ...
    x = x(:);
    % Evaluate:
    core = squeeze(ballfun.txm(f.core, feval(f.cols, x), 1));
    if ( rY == 1 || rZ == 1 )
        core = core.';
    end
...
elseif ( strcmpi(x, ':') && isnumeric(y) &&  strcmpi(z, ':') ) % f(:, y, :)
    ...
    y = y(:);
    % Evaluate:
    core = squeeze(ballfun.txm(f.core, feval(f.rows, y), 2));
    if ( rX == 1 )
        core = core.';
    end
...
elseif ( strcmpi(x, ':') && strcmpi(y, ':') && isnumeric(z) ) % f(:, :, z)
    ...
    z = z(:);
    % Evaluate:
    core = squeeze(ballfun.txm(f.core, feval(f.tubes, z), 3));
    out = chebfun2(f.rows * core.' * f.cols.');
    
elseif ( strcmpi(x, ':') && isnumeric(y) && isnumeric(z) )  % f(:, y, z)
    core = ballfun.txm(ballfun.txm(f.core, feval(f.rows,y), 2), ...
        feval(f.tubes,z), 3);
    out = f.cols * core;
    if ( isa(out, 'chebfun') ) 
        out = simplify(out); 
...
elseif ( isnumeric(x) && strcmpi(y, ':') && isnumeric(z) ) % f(x, :, z)
    core = ballfun.txm(ballfun.txm(f.core, feval(f.cols,x), 1), ...
        feval(f.tubes,z), 3)';
    out = f.rows * core;
    if ( isa(out, 'chebfun') ) 
        out = simplify(out); 
...
elseif ( isnumeric(x) && isnumeric(y) &&  strcmpi(z, ':') ) % f(x, y, :)
    core = squeeze(ballfun.txm(ballfun.txm(f.core, feval(f.cols, x), ...
        1), feval(f.rows, y), 2));
    out = f.tubes * core;
    if ( isa(out, 'chebfun') )
        out = simplify(out); 
...
for i = 1:size(xVec, 1) % TODO: How to vectorize this?
    outVec(i) = ballfun.txm(ballfun.txm(ballfun.txm(f.core,...
...
% way to evaluate a ballfuns. Check for this property. 
...
if ( max(max(max(abs(bsxfun(@minus, xeval, xeval(:, 1, 1)))))) == 0  && ...
    ...
    out = ballfun.txm(ballfun.txm(ballfun.txm(f.core, colsVals, 1),...
...
elseif ( max(max(max(abs(bsxfun(@minus, xeval, xeval(1, 1, :)))))) == 0 ...
    ...
    out = ballfun.txm(ballfun.txm(ballfun.txm(fCore, colsVals, 3),...
...
elseif ( max(max(max(abs(bsxfun(@minus, xeval, xeval(1, :, 1)))))) == 0  && ...
    ...
    out = ballfun.txm(ballfun.txm(ballfun.txm(fCore, colsVals, 2),...
...
elseif ( max(max(max(abs(bsxfun(@minus, xeval, xeval(1, :, 1)))))) == 0  && ... 
    ...
    out = ballfun.txm(ballfun.txm(ballfun.txm(fCore, rowsVals, 1),...
    ...
    % This is slow!!! 
    xx1 = ballfun.unfold(xeval, 1).'; % Transposed to make a long matrix
    yy1 = ballfun.unfold(yeval, 1).'; % so that it's faster in MATLAB.
    zz1 = ballfun.unfold(zeval, 1).';
    fevalNew = zeros(n(2)*n(3), n(1));
    for i=1:n(1)
        fevalNew(:,i) = vectorCase(f, xx1(:,i), yy1(:,i), zz1(:,i));
    end
    out = ballfun.fold(fevalNew.', n, 1, [2 3]);
end
...
elseif ( max(max(max(abs(bsxfun(@minus, xeval, xeval(:, 1)))))) == 0  && ...                     
    ...
    rowsVals = f.rows(yeval);
    tubesVals = f.tubes(zeval);
    out = squeeze(ballfun.txm(ballfun.txm(ballfun.txm(f.core, colsVals, 1),...
        rowsVals, 2), tubesVals, 3));
...
elseif ( max(max(max(abs(bsxfun(@minus, xeval, xeval(1, 1)))))) == 0  && ...                     
    ...
    rowsVals = f.rows(yeval);
    tubesVals = f.tubes(zeval);
    out = squeeze(ballfun.txm(ballfun.txm(ballfun.txm(f.core,...
