...
%   ballfun object. A ballfun object is a low-rank representation
...
%
%   The algorithm for constructing a ballfun has three phases:
%
...
%
% See also CHEBFUN2, ballfunT and ballfunV.
...
while ( ~isHappy && ~failure )
    ...
    % Users who want to create a ballfun from a DISCRETE tensor of values 
    ...
    if ( numel(pivotVals3D) > maxRank )
        warning('CHEBFUN:ballfun:constructor:rank', ...
    ...
    while ( (~isHappy) && (~failure) )
        ...
        if ( max([gridX, gridY, gridZ]) >= maxSample )
            warning('CHEBFUN:ballfun:constructor:notResolved', ...
                'Unresolved with maximum ballfun length: %u.', maxSample);
            failure = 1;
        end
        grid = min([m, n, p]);
    end
    ...
    % Construct a ballfun object, simplify skeletons and call a
    % sampleTest.
...
if ( flag ==1 )
    ...
    if ( size(vals) ~= size(xx) )
        % Necessary especially when a CHEBFUN2 is made out of a ballfun,
        % e.g. in ballfun/STD.
        vals = vals.';
    end
end
...
elseif ( isa(tech, 'chebtech1') )
    ...
    error('CHEBFUN:ballfun:constructor:gridRefinePhase1:techType', ...
...
elseif ( isa(tech, 'chebtech1' ) )
    ...
    error('CHEBFUN:ballfun:constructor:gridRefinePhase2:techType', ...
        'Technology is unrecognized.');
end
...
for k = 1:length(varargin)
    ...
    elseif strcmpi(varargin{k}, 'eps')
        pref.cheb3Prefs.ballfuneps = varargin{k+1};
...
% Fix the rank of a ballfun. Used for calls to constructor with specified 
% rank.

if ( any(fixedRank) < 0 )
    error('CHEBFUN:ballfun:constructor:fixTheRank:negative', ...
...
pseudoLevel = pref.cheb3Prefs.ballfuneps;
tech = pref.tech();
...
% Construct a ballfun object and call a sampleTest.
if ( ~isEqui )
    f.cols = chebfun(colsValues, dom(1:2), pref);
    f.rows = chebfun(rowsValues, dom(3:4), pref);
...
%   temp = zeros(size(A2)); 
%   for i = 1:3, 
%    temp = temp + ballfun.outerProd(slices(:,:,i),fibers(:,i)./pivotValues3D(i));
%   end
%   norm(AA(:) - temp(:))
%
...
while ( ( infNorm > tol ) && ( iter < width / factor) ...
    ...
    % A = A - ballfun.outerProd(slices(:,:,iter+1),fibers(:,iter+1)./PivVal3D);
