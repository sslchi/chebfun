...
if ( contractionMode == 1 )
    ...
    T_mat = ballfun.unfold(T, contractionMode, unContractedModes);
else
    % T will be transposed.
    T_mat = ballfun.unfold(T, unContractedModes, contractionMode);
end

if ( size(A, 2) ~= size_T(contractionMode) )
    error('CHEBFUN:ballfun:txm', 'Tensor-matrix dimensions do not agree');
end
...
% Fold T_mat
if ( contractionMode == 1 )    
    T = ballfun.fold(T_mat, size_T, contractionMode, ...
        unContractedModes);
else
    T = ballfun.fold(T_mat, size_T, unContractedModes, ...
        contractionMode);
end
